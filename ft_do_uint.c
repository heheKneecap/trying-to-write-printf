...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/08 15:14:15 by mshchuts          #+#    #+#             */
/*   Updated: 2023/03/08 15:14:24 by mshchuts         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_input(char *u_num, t_flags flags)
{
	...
	count = 0;
	if (flags.dot >= 0)
		count += ft_do_width(flags.dot - 1, ft_strlen(u_num) - 1, 1);
...
static int	ft_put_uint(char *u_num, t_flags flags)
{
	...
	if (flags.dot >= 0)
	{
		...
		count += ft_do_width(flags.width, ft_strlen(u_num), flags.zero);
	if (flags.minus == 0)
		count += ft_input(u_num, flags);
	return (count);
}
